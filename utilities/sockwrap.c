...
ssize_t readline_unbuffered (int fd, void *vptr, size_t maxlen)
{
	...
	for (n=1; n<maxlen; n++)
	{
		...
		{	
			//added by Giovanni Garifo <giovanni.garifo@polito.it>
			//if( n == 1 && c=='\0' ) continue;		

			printf("[debug - readline_unbuffered] letto da socket il byte numero %d: %c\n", n, c);

			*ptr = c;
			ptr++; //added by Giovanni Garifo <giovanni.garifo@polito.it>
			if (c == '\n')
				break;	/* newline is stored, like fgets() */
		}
		else if (rc == 0)
	...
	*ptr = 0; /* null terminate like fgets() */
	...
	printf("[debug - readline_unbuffered] stringa completa: %s\n", ptr);
